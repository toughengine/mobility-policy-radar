...
# 1차 필터: 이 대분류는 제목 키워드가 하나만 걸려도 채택한다.
TARGET_LARGE = {"연구조사서비스"}
# 이 대분류는 아예 보지 않는다 (토목 설계·감리, 폐기물, 청소 등).
SKIP_LARGE = {"기술용역", "폐기물 처리 및 재활용서비스", "시설물관리 및 청소서비스"}

# 2차 필터: 제목에 이 말이 있어야 채택. 기술경영·경제·통계·산업분석 계열로 좁혔다.
POS = [
    "정책연구", "기획연구", "연구용역", "학술연구",
    "산업분석", "산업동향", "시장분석", "시장조사",
    "경제성 분석", "경제성 평가", "경제적 파급", "파급효과",
    "타당성 조사", "타당성조사", "사전타당성",
    "실태조사", "현황조사", "수요조사", "인식조사",
    "기술수준", "기술동향", "기술로드맵", "특허분석", "기술가치", "기술사업화",
    "성과분석", "성과평가", "성과관리체계", "정책평가", "제도개선", "개선방안",
    "육성방안", "활성화 방안", "발전방안", "진흥계획",
    "중장기 발전", "중장기 전략", "경영전략", "경영진단", "조직진단", "조직개편",
    "비전 수립", "전략 수립", "전략수립", "로드맵 수립", "마스터플랜",
    "통계 개발", "통계 작성", "지표 개발", "지표체계",
    "실태 분석", "동향 분석", "밸류체인", "가치사슬", "생태계 분석",
    "컨설팅", "자문 용역", "기본구상", "기본연구",
]

# 제목에 이 말이 있으면 버린다. 실측에서 실제로 섞여 들어온 것들만 넣었다.
NEG = [
    # 시험·측정·분석 (연구조사서비스 안에 섞여 있다)
    "시험분석", "성분분석", "시료", "토양", "수질", "대기측정", "환경측정",
    "검교정", "교정", "Test", "시험평가", "실험", "검사", "정밀안전진단", "내진", "안전진단",
    # 토목·건축·시설 — '기본계획/종합계획'이 이쪽을 대량으로 끌어온다
    "소하천", "하천정비", "하수도", "하수관로", "상수도", "정수장", "우수관", "저수지",
    "배수", "제방", "도로정비", "지형도면", "전략환경영향평가", "지구단위", "도시관리계획",
    "재해예방", "사방", "항만", "어항", "산림경영", "산사태", "유아숲", "생태복구", "식물상",
    "설계용역", "감리", "건설사업관리", "측량", "지질조사", "시설물", "기술진단",
    "보수", "정비공사", "전기공사", "통신공사", "조경", "수목",
    # 운영·관리 위탁
    "방역", "소독", "청소", "경비", "급식", "폐기물", "임차", "임대", "대여",
    "유지관리", "운영 위탁", "민간위탁", "위탁기관 선정", "보험", "여행", "숙박",
    "행사 대행", "축제", "공간 설계", "공간 활용", "기자재",
    # 결이 다른 컨설팅
    "정보보호", "개인정보", "ISMS", "정보보안", "위험성평가", "자체감사", "PSM",
    "특허사무소", "PMC", "봉사단", "문항", "면접", "인쇄", "제작 및 설치",
]
...
def keep(row: dict) -> bool:
    ...
    if any(n in title for n in NEG):
        return False
    large = row.get("pubPrcrmntLrgClsfcNm")
    if large in SKIP_LARGE:
        return False
    if not any(p in title for p in POS):
        return False
    return True    # 대분류가 TARGET_LARGE든 아니든, POS를 통과했으면 채택
