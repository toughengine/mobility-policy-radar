...
## 수집 경로와 그 한계 (반드시 이해할 것)

| 레이어 | 소스 | 이 세션에서 가능? |
|---|---|---|
| A. API 수집 | `apis.data.go.kr` (부처 보도자료, 나라장터 용역공고) | **간헐적** — 터널이 자주 리셋됨. 자동화에서 의존하지 말 것. 사용자가 로컬에서 `scripts/collect_datago.py` 실행 |
| B. 검색 수집 | WebSearch | **가능** — 이 자동화의 주 수집 경로 |
| C. 분석 | Claude | **가능** — 이 저장소의 핵심 가치 |

NTIS·IRIS·SROME·빅카인즈·정책브리핑(korea.kr)·부처 홈페이지는 이 세션의 egress 정책에서
**403으로 차단**됩니다. WebFetch로도 열리지 않습니다. 따라서 이들 사이트의 내용은
**WebSearch 결과를 통해서만** 확보하며, 직접 접속을 시도하지 않습니다.
...
2. **수집 (WebSearch)** — 아래 쿼리를 기본으로 실행하고, 그날 이슈에 맞춰 추가한다:
   - `자율주행 신규사업 공고 2026` / `모빌리티 R&D 신규과제 공고`
   - `국토교통부 보도자료 자율주행 모빌리티`
   - `산업통상부 자동차 R&D 공고` / `KEIT 자동차산업기술개발사업 공고`
   - `NTIS 국가R&D통합공고 자율주행` / `기술수요조사 모빌리티`
   - `예비타당성조사 통과 모빌리티 자동차` / `UAM 사업 공고`
   - `전기차 배터리 기술개발 사업 공고`

   **선행 신호를 우선한다.** 이미 마감된 공고보다 *기술수요조사·연구기획사업·업무계획·
   예타 통과*가 훨씬 가치 있다 — 이들이 1~3년 뒤 신규사업의 예고편이기 때문이다.
...
- NTIS·IRIS·SROME·korea.kr 등 차단 도메인에 WebFetch/직접 접속 시도 (403, 시간 낭비)
