...
설계 근거 (2026-09-23 실측, 7일치 3,410건 전수 분석):
  ...
  * 조건을 'A(연구 형태) AND B(산업 분야)'로 걸어 50건/7일(하루 7건)로 수렴했다.
...
# 대분류 1차 — 이쪽은 아예 보지 않는다.
# '기술용역'은 한때 여기 있었으나 뺐다. 교통연구원 「화물자동차 안전운임제 성과분석 및
# 발전방안 마련 연구」처럼 명백한 정책연구가 조달 분류상 기술용역으로 올라오기 때문이다.
# 대신 그 대분류에서 딸려오는 「설계의 경제성(VE) 검토」류는 NEG로 막는다.
SKIP_LARGE = {"폐기물 처리 및 재활용서비스", "시설물관리 및 청소서비스"}
...
WORK = [
    ...
    # 아래는 실측에서 빠진 표현을 메운 것이다. KEIT·KIAT·교통연구원의 정책연구가
    # 「~ 방안 연구」·「전략체계 고도화」·「사업 기획 연구」 같은 제목으로 나오는데
    # 기존 목록으로는 한 건도 걸리지 않았다.
    "방안 연구", "방안 마련", "방안 도출", "체계 고도화", "전략체계",
    "영향 분석", "사전 분석", "기획 연구", "정책과제",
]
...
NEG = [
    ...
    # 기술용역 대분류를 열면서 딸려온 것들 (실측)
    "(VE)", "실시설계", "기초학력", "학습지원", "장병", "입도", "다이캐스팅", "공정데이터",
    # 분야가 다른 조사
